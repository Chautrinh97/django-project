...
(function ($) {
    ...
    $(document).ready(function () {
        $(".product-active").each(function () {
            if ($(this).find('.single-product').length >= 5) {
                $(this).owlCarousel({
                    loop: true,
                    nav: true,
                    dots: false,
                ...
                $(this).owlCarousel({
                    loop: false, // Bỏ loop
                    nav: $(this).children.length > 1, // Nếu số lượng sản phẩm > 1 thì hiện nút nav
                    dots: false,
                    autoplay: false,
                    navText: ["<i class='fa fa-angle-left'></i>", "<i class='fa fa-angle-right'></i>"],
...
function showProductDetail(productId) {
    $.ajax({
        ...
        success: function (response) {
            $('#productPreviewId').val(response.id);
            $('#productPreviewName').text(response.name);
            $('#productPreviewBrand').text(response.brand);
            $('#productBrandLink').attr('href', '/collections/brand/' + response.brslug);
            $('#productPreviewDescription').text(response.description);
            $('#productPreviewPrice').text(new Intl.NumberFormat('vi-VN', {
                style: 'currency',
                currency: 'VND'
            }).format(response.price));
            $('#previewProductImage').attr('src', response.images[0]);
            $('#previewProductImage').attr('data-zoom-image', response.images[0]);
            $('#productModal').modal('show');

            $('#productPreviewAddToCart').click(function(event){
                event.preventDefault();
                var productId = $('#productPreviewId').val();
                var quantity = $('#productPreviewQuantity').val();
...
function onOrderChange(element) {
...
function clearFilter() {
    ...
    var orderSelector = document.querySelector('#orderBy');
    orderSelector.selectedIndex = 0;
    document.querySelector("#filterForm").submit();
}

function singleProductAddToCart(productId){
    var quantity = $('#singleProductQuantity').val();
    addToCart(productId, quantity);
}
...
function updateMiniCart(cart) {
    var $miniCartProducts = $('#miniCartProduct');
    $miniCartProducts.empty();
    var cartKeys = Object.keys(cart);
    var lastThreeKeys = cartKeys.length >= 3 ? cartKeys.slice(-3) : cartKeys;
    for (var i = 0; i < lastThreeKeys.length; i++) {
        var key = lastThreeKeys[i];
        var product = cart[key];
        var $cartItem = $('<li></li>');
        var $productImage = $('<img src="' + product.imageURL + '" alt="cart products">');
        var $productDetails = $('<div class="minicart-product-details"></div>');
        var $productName = $('<h6><a href="products/' + product.slug+ '">' + product.name + '</a></h6>');
        var $productPrice = $('<span></span>').text(new Intl.NumberFormat('vi-VN', {
                style: 'currency',
        ...
        $cartItem.append($('<a href="products/'+ product.slug +'" class="minicart-product-image"></a>').append($productImage));
        $cartItem.append($productDetails);

        $miniCartProducts.append($cartItem);
    }
    $('#cartItemCount').text(cartKeys.length);
}
